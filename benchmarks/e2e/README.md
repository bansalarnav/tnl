...
The script runs four matched cases three times: concurrency-one latency, 1 KiB responses, 1 MiB responses, and fresh connections with keep-alive disabled. Override the defaults with `DURATION` and `REPETITIONS`.

To run a smaller experiment matrix, set `BENCH_CASES` to semicolon-separated `name size concurrency mode` entries. For example:

```sh
BENCH_CASES='bulk-c1 1048576 1 keepalive;bulk-c16 1048576 16 keepalive'
```
