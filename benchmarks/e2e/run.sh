...
printf '%s\n' 'run,path,case,size_bytes,concurrency,keepalive,requests_per_sec,mean_ms,p50_ms,p95_ms,p99_ms,success_rate' > "$summary_csv"
printf '%s\n' 'run,path,case,process,pid,cpu_percent_of_one_core,peak_rss_kib' > "$process_csv"

default_cases=(
  'latency 0 1 keepalive'
  'small 1024 32 keepalive'
  'bulk 1048576 16 keepalive'
  'new_connection 0 8 new'
)
...
run_case() {
  local run=$1 path=$2 case_name=$3 size=$4 concurrency=$5 connection_mode=$6
  local base url output started_ns finished_ns elapsed_ns load_pid
  ...
  jq -r \
    ...
    --arg size "$size" --arg concurrency "$concurrency" --arg keepalive "$connection_mode" \
    '[ $run, $path, $case, $size, $concurrency, $keepalive,
...
for ((run = 1; run <= repetitions; run++)); do
  for definition in "${cases[@]}"; do
    read -r case_name size concurrency connection_mode <<< "$definition"
    for path in "${paths[@]}"; do
      if [[ $path != direct && $path != tunnel ]]; then
        echo "BENCH_PATHS entries must be 'direct' or 'tunnel'" >&2
        exit 2
      fi
      run_case "$run" "$path" "$case_name" "$size" "$concurrency" "$connection_mode"
    done
  done
done
